...
export default function DepartmentProfileTableHOC(props) {
  ...
  const [facultyConfig, setFacultyConfig] = React.useState(
  ...
  if (loading) {
    return (
      <div style={{ width: "100%" }}>
        <center>
          <Typography>Please Wait While We Generate The Report</Typography>
          <CircularProgress />
        </center>
      </div>
    );
  ...
  return (
    ...
    >
      {store.state.user.type !== "FACULTY" && (
        <FormControl className={`${classes.formControl} print-friendly`}>
          <InputLabel id="demo-simple-select-label" className="print:hidden">
            Department
          </InputLabel>
          <Select
      ...
      {Object.keys(facultyConfig).map((key_) => (
        <>
          <Paper elevation={2} variant="outlined" className="print-docs">
            <TableContainer
