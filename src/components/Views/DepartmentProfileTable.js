    ...
    <Paper variant="outlined" elevation={1} className={classes.tableRoot}>
      <TableContainer>
        <div>
          ...
          >
            <b>{`${generateTabHeading(criterion, step)} ${
              FACULTY_PROFILE_CONFIG[criterion][step].description
            }`}</b>{" "}
          </Typography>
          <Divider />
        </div>
        <Table className={classes.table} aria-label="simple table">
          ...
          <TableBody>
            ...
            {!state.data[`fp${criterion}${step}`] && (
              <TableCell align="center">No Data Found</TableCell>
            )}
            {state.data[`fp${criterion}${step}`] &&
              state.data[`fp${criterion}${step}`].length === 0 && (
                <TableCell align="center">No Data Found</TableCell>
              )}
            {state.data[`fp${criterion}${step}`] &&
              state.data[`fp${criterion}${step}`].map(
                (row) =>
                  ((row["faculty_profile"] &&
                    parseInt(props.department) === 0) ||
                    parseInt(props.department) ===
                      parseInt(row["faculty_profile"]["dept"])) && (
                    <TableRow key={JSON.stringify(row)}>
                      {props.fields.map(
                        (field) =>
                          !EXCLUDE_FIELDS[field.name] && (
                            <TableCell component="th" scope="row">
                              {FIELDS_WITH_NAME[field.name]
                                ? row[field.name].name
                                : row[field.name]}
                            </TableCell>
                          )
