...
const SelectField = (props) => {
  ...
  useEffect(() => {
    if (field.props.options) {
      ...
      axios
        .get(`${GLOBAL_URL}/${field.props.field}`)
      ...
      >
        <Grid item xs={12} sm={6}>
          <FormControl component="fieldset">
            ...
            <RadioGroup
              aria-label={field.label.replaceAll("_", " ")}
              name={field.name}
              row
              value={value}
...
function Step(props) {
  ...
  const { state, addBtnFaculty, removeBtnFaculty } =
  ...
  const handleSubmit = (e) => {
    ...
    console.log(data);
    addBtnFaculty(criterion, step, data);
  };
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {state.data[`fp${criterion}${step}`] &&
                state.data[`fp${criterion}${step}`].map((row) => (
