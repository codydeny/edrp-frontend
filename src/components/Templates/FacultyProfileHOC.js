...
export default function FacultyProfileHOC() {
  ...
  const [criterionConfig, setCriterionConfig] = React.useState(
    FACULTY_PROFILE_CONFIG
  );
    ...
    if (
      ...
      Object.keys(CRITERION_CONFIGURATION).map((criterionKey) => {
        ...
        ALLOWED_FACULTY_STEPS[criterionKey].map((stepKey) => {
          allowedSteps = {
            ...allowedSteps,
            [stepKey]: CRITERION_CONFIGURATION[criterionKey][stepKey],
          };
        });
  ...
  return (
    ...
    >
      <Typography
        variant="h6"
        align="center"
        style={{ margin: "15px", text: "center" }}
      >
        {store.state.user.type === "HOD" ? "Department" : "Faculty"} Profile
        Module
      </Typography>
      <center>
        {Object.keys(criterionConfig).map(
          (key_) =>
            !shouldBeShownToFaculty(key_) && (
              <Chip
                key={key_}
                clickable
                style={{
                ...
                variant={key === key_ ? "default" : "outlined"}
                color="primary"
                icon={<AssignmentIcon />}
                label={`Module ${key_}`}
                onClick={() => handleClick(key_)}
              />
            )
        )}
      </center>
      ...
      {criterionConfig[key] && (
        <FacultyProfile
          criterion={criterionConfig[key]}
          criterionId={key}
          criterionDescription={FP_DESCRIPTION[key]}
        />
      )}
