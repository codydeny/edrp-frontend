...
class MyProvider extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      data: {},
      user: localStorage.getItem("iqac_user")
        ? JSON.parse(localStorage.getItem("iqac_user"))
        : {},
      facultyResume: {},
      facultyList: [],
    };
      ...
      <MyContext.Provider
        value={{
          ...
          resetPassword: async (e) => {
            axios
              .post("reset_password/", new FormData(e.target))
              .then((response) => {
          ...
          submitFacultyForm: async (formData, id, closeFunc) => {
            axios
              .patch(
                `/actual_faculty_profile_update/${id}/`,
          ...
          addBtn: async (critertion, step, entry) => {
            ...
            axios
              .post(`c${critertion}${step}/`, {
                ...entry,
                criterion: criterionId.id,
              })
              .then(async (res) => {
                console.log(criterionId);
                this.setState({
          ...
          addBtnFaculty: async (criterion, step, entry) => {
            let facultyProfile = await (
              await axios.get(`faculty_profile`)
            ).data;
            console.log(facultyProfile);
            axios
              .post(`fp${criterion}${step}/`, {
                ...entry,
                faculty_profile: facultyProfile.id,
              })
              .then(async (res) => {
                ...
                let fp = await (
                  await axios.get(`fp${criterion}${step}/${res.data.id}`)
                ).data;
          ...
          fetchAllStepsData: async () => {
            ...
            let arr = [
              "criterionone_detail",
              "criteriontwo_detail",
              "criterionthree_detail",
              "criterionfour_detail",
              "criterionfive_detail",
              "criterionsix_detail",
              "criterionseven_detail",
            ];
                  ...
                  Object.keys(individual_item).map((step) => {
                    if (step.includes("_set")) {
                      ...
                      let data = this.state.data[newKey]
                        ? this.state.data[newKey].concat(individual_item[step])
                        : individual_item[step];
                      this.setState({
                        data: {
                          ...this.state.data,
                          [newKey]: data,
